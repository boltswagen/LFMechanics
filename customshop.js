...
otherCountries.forEach(element => {
    element.addEventListener("click", () => {
        ...
        confirm("Custom orders over a certain weight cannot be shipped worldwide. Please confirm that your order is below the limit weight of 30 kg before proceeding.");

        // Entering weight
        const weightLabel = document.createElement("label");
        weightLabel.textContent = "Order Weight: ";
        const weightBox = document.createElement("input");
        weightBox.className = "textbox";
        weightBox.id = "weight-input";
        weightBox.placeholder = "No larger than 30 kg!";
        weightBox.required = true;
        ...
        addingElements.appendChild(document.createElement("p"));
        addingElements.appendChild(weightLabel);
        addingElements.appendChild(weightBox);
        addingElements.appendChild(document.createElement("p"));
...
function validateForm(){
    ...
    if (selectedCountry !== "Canada"){
        if (enteredWeight > 30){
            alert("Order weight cannot exceed 30 kg.");
            return false;
        }
    }

    // if all valid, clear page and redirect to home
...
// On submit action
document.getElementById("custom-form").onsubmit = validateForm;